...
### Added
- A second documentation site, `handbook`, served below `/site/handbook/` and published without an architecture model.
- The handbook in the site suite: left alone by the import, published by an operator and by an upload naming it, kept apart from the default site, with its own colour scheme and search.
- An upload for a site the instance does not configure, refused with `UNKNOWN_SITE`.
